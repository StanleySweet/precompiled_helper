...
class IncludeParser(Preprocessor):
	def __init__(self, file_to_parse, include_dirs, directly_includes, included_directly_by, includes, included_by):
		# For internal consistency, we throw if the file to be parsed doesn't exist.
		...
		self.includes = set()
		self.directly_includes = set()
		directly_includes[self.file_path] = self.directly_includes
		includes[self.file_path] = self.includes
	...
	def handle_header(self, header):
		if header == self.file_path:
			return
		if self.curr_file == self.file_path:
			self.directly_includes.add(header)
			if header not in self.included_directly_by:
				self.included_directly_by[header] = set()
			self.included_directly_by[header].add(self.file_path)
