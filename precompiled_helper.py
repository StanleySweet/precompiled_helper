...
def parse_args():
	...
	if args.file:
		...
		remove_headers_from_precompiled = config["remove_headers_from_precompiled"]
	...
	with open('scores.csv','w') as out:
		...
		for f in included_by:
			if prune_solo and f in included_directly_by and len(included_directly_by[f]) ==1:
				continue
			out.write(f'{f};{len(included_by[f])};{len(included_directly_by[f])};{len(included_by[f])/len(files)};{(len(directly_includes[f]) if f in directly_includes else 0)*len(included_by[f])};{0 if f in directly_includes else 1}\n')
